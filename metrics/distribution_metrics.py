...
def calculate_gradient_statistics(
    ...
    Args:
        gradient_tensor: 梯度張量。
        quantiles: 需要計算的分位數列表，預設為 [0.25, 0.5, 0.75]。
    ...
    if not isinstance(gradient_tensor, torch.Tensor):
        raise TypeError("輸入必須是 PyTorch Tensor")

    # 將張量展平以便計算統計量
    flat_tensor = gradient_tensor.detach().cpu().numpy().flatten()
    ...
    stats_dict = {
        ...
        'norm': float(np.linalg.norm(flat_tensor)) # 使用 NumPy 計算 L2 範數
    }

    # 計算分位數
    if quantiles:
        q_values = np.quantile(flat_tensor, quantiles)
        for q, val in zip(quantiles, q_values):
            stats_dict[f'q_{q}'] = float(val)
            # 特別標註中位數
            if q == 0.5:
                stats_dict['median'] = float(val)


    return stats_dict


def calculate_gradient_histogram(
    gradient_tensor: torch.Tensor,
    bins: int = 100,
    density: bool = True
    ...
    Args:
        gradient_tensor: 梯度張量。
        bins: 直方圖的區間數量。
        density: 是否將直方圖標準化為概率密度。

    Returns:
        包含直方圖值和區間邊界的元組 (hist, bin_edges)。
    """
    # 確保輸入是 Tensor
    if not isinstance(gradient_tensor, torch.Tensor):
        raise TypeError("輸入必須是 PyTorch Tensor")
    ...
    # 計算直方圖
    hist, bin_edges = np.histogram(flat_tensor, bins=bins, density=density)
